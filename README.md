...
**edAutoMarket v2.0** is an advanced *Auto Trading* (Auto Buy/Sell) modification for the game **Stronghold Crusader HD**. This mod allows you to set stock thresholds for goods in your granary and armory; if a resource stock exceeds the sell threshold, it will be automatically sold. If it falls below the buy threshold, it will be automatically purchased.

This mod is a major fork of **Ciptik's** original source code, focusing on keyboard-driven navigation efficiency, a dual profile (*Snapshot*) system, cleanly categorized items, full UI theme customization, and DirectDraw proxy stability for 100% compatibility with other popular modifications.
...
* 📁 **Dual Config File System:**
  * `automarket.ini`: Stores main hotkey settings, UI theme customization, and default values.
  * `automarketsave.ini`: Stores instant threshold *snapshot* profiles that can be loaded/saved mid-game.
* 🎨 **Fully Customizable UI Theme:** Customize menu colors (RGB), font family, font sizes, menu width, row spacing, and screen position offsets via `automarket.ini`.
...
| `CTRL + L` | **Reload Config:** Re-reads `automarket.ini` from disk and applies new hotkeys, colors, fonts, and layout instantly. |
| *(Automatic)* | When a match/skirmish ends (win/lose/quit), the mod will **automatically reset** all values to *defaults*. |

---

## ⚙️ Configuration & Customization (`automarket.ini`)

All hotkeys, trade speeds, colors, and layout dimensions can be customized in **`automarket.ini`** (located in your game root directory). Press **`CTRL + L`** in-game to apply any changes made to this file in real-time.

```ini
[Hotkeys]
; Decimal Virtual-Key Codes (77 = M, 83 = S, 76 = L, 82 = R)
ToggleMenu=77
ToggleMenu_Ctrl=0
ToggleMenu_Shift=0
...
; Font Options (e.g. Consolas, Arial, Tahoma, Verdana)
FontName=Consolas
FontSize=16
TitleSize=22

; UI Element Colors (RGB Format: Red,Green,Blue)
BgColor=20,20,38
HeaderColor=50,35,80
RowColor=30,32,55
SelColor=0,110,180
EditColor=190,60,30
CatColor=25,65,75

; Text Colors (RGB Format: Red,Green,Blue)
TitleColor=255,215,0
TextColor=210,240,255
HighlightTextColor=255,255,200
EditTextColor=100,255,100
